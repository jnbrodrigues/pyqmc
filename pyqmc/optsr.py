...
def power_gradient_descent(wf,coords,pgrad_acc,warmup=10,
        step=0.5,eps=0.1,maxiters=50,
        vmc=None,vmcoptions=None,
        datafile=None,verbose=2,power=0.5):
    
    """Optimizes energy using gradient descent with stochastic reconfiguration.
       Uses the power gradient descent proposed by M. Baiesi in arXiv:1906.04787.
    ...
    Args:
      ...
      power: 0 < scalar < 1 to be used as pgrad_i = sign(pgrad_i) * |pgrad|^power  
    ...
    # Gradient descent cycles
    for it in range(maxiters):
        Sgrad = np.einsum('ij,j->i',invSij,pgrad)
        Sgrad=np.sign(Sgrad)*np.power(np.abs(Sgrad),power) # Power gradient
        x0 -= Sgrad * step/(it/10+1)
        pgrad,pgrad_std,invSij,en,en_std,nsteps = gradient_energy_function(x0)
        if verbose > 1:
            print('p =',x0)
            print('grad =',pgrad)
...
def test():    
    ...
    mol = gto.M(atom='H 0. 0. 0.; H 0. 0. 1.5', basis='bfd-vtz',unit='bohr',verbose=5)
    mf = scf.RHF(mol).run()
    nconf=2500
    nsteps=150
    warmup=30
    coords0 = initial_guess(mol,nconf)
    ...
    wf,data=gradient_descent(wf,coords,pgrad_acc,
            ...
            step=0.5,eps=0.1,maxiters=40,datafile='sropt.json',verbose=1)
    t1=time.time()
    ################################
    print('h-GD method:')
    coords = coords0.copy()
    ...
    pgrad_acc=PGradTransform(energy_acc,LinearTransform(wf.parameters))
    # Gradient descent
    wf,data=power_gradient_descent(wf,coords,pgrad_acc,
            vmcoptions={'nsteps':nsteps},warmup=warmup,
            step=0.5,eps=0.1,maxiters=40,datafile='sropt.json',verbose=1,power=0.75)
    t2=time.time()
    ################################
    print('GD time:',t1-t0)
    print('h-GD time:',t2-t1)
